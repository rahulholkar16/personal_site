  ...
  return (
    <div className="cv-page relative z-10 max-w-3xl mx-auto px-6 py-16 sm:py-20">
      ...
      <header className="mb-12 flex flex-col sm:flex-row items-start gap-6 sm:gap-8">
        {/* Initials avatar */}
        <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-full bg-(--color-accent) flex items-center justify-center shrink-0">
          <span className="display text-4xl text-white leading-none">RP</span>
        </div>
        <div className="flex-1">
          <h1 className="display text-5xl sm:text-6xl leading-[0.95] tracking-tight mb-2">
            Rahul Pal<span className="text-(--color-accent)">.</span>
          </h1>
          <p className="text-lg text-(--color-text-secondary) mb-2">
            <span className="display-italic">Full Stack Developer</span> focused on
            building scalable, user-first products.
          </p>
          ...
          {PROJECTS.map((project) => (
            <div key={project.name} className="cv-block">
              <div className="flex flex-wrap items-baseline justify-between gap-x-4 mb-1">
                <h3 className="display text-xl text-(--color-text)">{project.name}</h3>
                <div className="flex items-center gap-3">
                  ...
                  >
                    {project.sourceUrl ? "Source Code ↗" : "Live Demo ↗"}
                  </a>
                </div>
              </div>
      ...
      <section className="mb-4">
        ...
        <ul className="space-y-3">
          {[
            ...
            <li key={item.title} className="flex gap-2 text-sm text-(--color-text-secondary)">
              <span className="text-(--color-text-muted) shrink-0">—</span>
              <span>
                <strong className="text-(--color-text) font-medium">{item.title}:</strong>{" "}
                {item.description}
              </span>
            </li>
          ))}
